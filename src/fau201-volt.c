...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        if (voltage < 0 || voltage > 4.095)  // If the obtained voltage value (in V) after conversion is lesser than 0 or greater than 4.095
        {
            fprintf(stderr, "Error: Voltage should be between 0 and 4.095V.\n");
            err_level = EXIT_USERERR;
        }
        else
        {
            ...
            else  // If libusb is initialized
            {
                ...
                if (argc == 2)  // If no serial number was specified
                    devhandle = libusb_open_device_with_vid_pid(context, 0x10C4, 0x8C46);  // Open a device and get the device handle
                else  // Serial number was specified as a second (optional) argument
                    devhandle = libusb_open_device_with_vid_pid_serial(context, 0x10C4, 0x8C46, (unsigned char *)argv[2]);  // Open the device having the specified serial number, and get the device handle
                ...
                else  // If the device is successfully opened and a handle obtained
                {
                    ...
                    else  // If the interface is successfully claimed
                    {
                        ...
                        setup(devhandle);  // Configure the DAC, so that it uses the external voltage reference (by sending a specific sequence of three bytes to the LTC2640 DAC on channel 0)
                        ...
                        set_voltage(devhandle, volt_code);  // Set the output voltage to the intended value (again, by sending a sequence of three bytes the LTC2640 DAC on channel 0)
